"""
多语言支持
"""

TRANSLATIONS = {
    "zh": {
        # 主窗口
        "window_title": "Smoco Desktop",
        # 设备选择页
        "select_device": "选择音频设备",
        "select_device_desc": "选择要采集的扬声器设备：",
        "start_transcription": "开始转录",
        "refresh_devices": "刷新设备列表",
        "no_devices": "未找到音频设备",
        "loading_devices": "加载设备中...",
        # 转录页
        "realtime_transcript": "实时转录",
        "stop": "停止",
        "waiting": "等待转录结果...",
        # 错误
        "no_device_selected": "请先选择设备",
        "start_failed": "启动失败",
        "audio_error": "音频错误",
        "audio_capture_failed": "音频采集失败：",
        # 语言选择
        "language": "语言",
        "language_zh": "简体中文",
        "language_ja": "日本語",
        "language_en": "English",
        # 设置
        "settings": "设置",
        "whisper_servers": "Whisper 服务器列表",
        "add_server": "添加",
        "remove_server": "删除",
        "set_default": "设为默认",
        "new_server_url": "新服务器 URL:",
        "server_name_optional": "名称（可选）:",
        "server_url_placeholder": "http://your-server:port",
        "server_name_placeholder": "可选：服务器名称",
        "vad_parameters": "VAD 参数",
        "silence_duration": "静音时长",
        "max_chunk_duration": "最大块时长",
        "min_chunk_duration": "最小块时长",
        "padding_duration": "填充时长",
        "restore_defaults": "恢复默认",
        # 本地 Whisper
        "local_whisper_model": "本地 Whisper 模型 (GPU)",
        "local_model_size": "模型类型",
        "local_port": "服务端口",
        "local_device": "运行设备",
        "local_device_auto": "自动检测 (优先 GPU)",
        "local_device_gpu": "GPU (性能最好)",
        "local_device_cpu": "CPU (兼容模式)",
        "local_cpu_warning": "注意：Whisper 模型对音源质量非常敏感——只有当用户靠近麦克风说话，或使用耳机自带的麦克风时，才能获得较好的识别效果。此问题将在后续接入 Smoco 实时语音转录功能后得到改善。",
        "local_start": "启动服务",
        "local_stop": "停止服务",
        "local_status": "状态",
        "local_status_stopped": "已停止",
        "local_status_starting": "启动中...",
        "local_status_running": "运行中",
        "local_status_downloading": "下载模型中...",
        "local_already_running": "服务已在运行",
        "local_starting": "正在启动服务",
        "local_running": "运行中",
        "local_stopped": "已停止",
        "local_start_timeout": "启动超时，请检查日志",
        "local_server_name": "本地 Whisper 服务",
        "local_model_small_label": "Small OpenVINO (INT8)",
        "local_model_small_desc": "速度快、占用低，适合日常使用",
        "local_model_turbo_label": "Large v3-Turbo (INT8)",
        "local_model_turbo_desc": "更高准确率，需更多显存/内存",
        "local_model_recommended": "推荐",
        "local_model_missing": "所选模型文件未找到，请重新安装或检查模型目录",
        "local_model_switch_restart": "已切换模型，重启服务后生效",
        "save": "保存",
        "cancel": "取消",
        "hint_enter_url": "请输入服务器 URL",
        "hint_server_exists": "该服务器已存在",
        "hint_cannot_delete_default": "无法删除默认服务器，请先设置其他服务器为默认",
        "hint_select_server": "请先选择一个服务器",
        "hint_need_server": "请至少添加一个 Whisper 服务器",
        # 启动对话框
        "startup_title": "启动转录",
        "select_whisper_server": "选择 Whisper 服务器:",
        "current_server": "当前服务器:",
        "service_status": "服务状态",
        "service_checking": "检查中...",
        "checking": "检查中",
        "service_normal": "服务正常",
        "service_error": "服务错误",
        "service_timeout": "连接超时",
        "service_connection_error": "无法连接到服务器",
        "service_check_failed": "检查失败",
        "select_language": "选择转录语言:",
        "select_translate_language": "选择翻译语言:",
        "enable_prompt": "启用提示（可能拖慢识别）",
        "japanese": "日语",
        "english": "英语",
        "chinese": "中文",
        "speaker": "扬声器",
        "microphone": "麦克风",
        # LLM 配置
        "llm_config": "LLM 配置",
        "llm_base_url": "Base URL:",
        "llm_api_key": "API Key:",
        "llm_model": "模型 ID:",
        "llm_validate": "验证配置",
        "llm_validating": "验证中...",
        "llm_config_ok": "配置正确",
        "llm_config_error": "配置错误",
        "llm_config_incomplete": "配置不完整",
        "llm_config_timeout": "连接超时",
        "llm_config_connection_error": "无法连接到服务器",
        "llm_config_failed": "验证失败",
        "llm_not_configured": "LLM 未配置",
        "llm_invalid_response": "响应格式无效",
        "llm_json_parse_error": "JSON 解析失败",
        "llm_timeout": "翻译超时",
        "llm_base_url_placeholder": "https://api.openai.com/v1",
        "llm_api_key_placeholder": "sk-...",
        "llm_model_placeholder": "gpt-4o",
        "llm_instructions": "说明：OpenAI 兼容的 API 服务",
        "llm_use_default": "使用缺省",
        "smoco_use_default": "使用缺省",
        "smoco_source_name": "Smoco 云端 ASR",
        "smoco_stt_config": "Smoco 云端 STT",
        "smoco_instructions": "说明：使用 smoco 账号登录云端流式语音识别（socket.io）",
        "smoco_host": "服务器地址",
        "smoco_host_placeholder": "https://dx-smoco-dev.sony.com.cn",
        "smoco_email": "账号 (Email)",
        "smoco_password": "密码",
        "smoco_use_punctuator": "自动标点",
        "smoco_insecure_ssl": "关闭 SSL 证书校验（内部 CA）",
        "smoco_validate": "验证登录",
        "smoco_validating": "验证中...",
        "smoco_validate_ok": "登录成功",
        "smoco_validate_failed": "登录失败",
        "smoco_not_configured": "Smoco 账号未配置，请在设置里填写",
        "cannot_close_while_transcribing": "正在转录中，请先停止转录再关闭窗口",
        "smoco_account_unavailable": "Smoco 账户当前不可用，请检查设置中的账号或网络",
        "llm_context_length": "翻译上下文条数:",
        "llm_context_length_tooltip": "翻译时使用的前几条转录条目作为上下文，默认5条",
        "start": "开始转录",
        "service_unavailable": "服务不可用",
        "service_unavailable_msg": "无法连接到 API 服务，请选择其他服务器或检查配置",
        # 转录交互
        "copy_transcript": "复制转录文本",
        "copy_with_timestamp": "复制带时间戳",
        # 通用
        "hint": "提示",
        "warning": "警告",
        "error": "错误",
        "success": "成功",
        "failed": "失败",
        "loading": "加载中...",
        "please_wait": "请稍候",
        # 错误消息
        "need_server_config": "请先在设置中添加 Whisper 服务器",
        "unable_to_start": "无法启动转录：",
        # 转录历史
        "history": "历史",
        "history_title": "转录历史",
        "history_back": "返回",
        "history_loading": "加载中...",
        "history_no_sessions": "暂无转录历史",
        "history_no_entries": "该会话无转录记录",
        "history_entry_count": "{n} 条",
        "history_prev_page": "上一页",
        "history_next_page": "下一页",
        "history_page_n_of_m": "第 {n} / {m} 页",
        "history_export": "导出",
        "history_export_this": "导出此会话",
        "history_export_format": "导出格式:",
        "history_export_done": "已导出到: {path}",
        "history_copy_original": "复制原文",
        "history_copy_translation": "复制译文",
        "history_copy_done": "已复制到剪贴板",
        "history_no_translation": "（无翻译）",
        "history_close": "关闭",
        "history_time_column": "时间",
        "history_original_column": "原文",
        "history_translation_column": "译文",
        "history_detail_title": "转录详情 #{id}",
        # 日志查看
        "logs": "日志",
        "logs_title": "应用日志",
        "refresh_logs": "刷新",
        "open_log_dir": "打开日志目录",
        "error_logs_only": "仅错误日志",
        "no_logs": "暂无日志文件",
        "loading_logs": "加载中...",
        "log_status": "显示最近 {shown} / {total} 行 · 文件大小 {size}",
    },
    "ja": {
        # 主窗口
        "window_title": "Smoco Desktop",
        # 设备选择页
        "select_device": "オーディオデバイスを選択",
        "select_device_desc": "録音するスピーカーデバイスを選択：",
        "start_transcription": "文字起こし開始",
        "refresh_devices": "デバイスリストを更新",
        "no_devices": "オーディオデバイスが見つかりません",
        "loading_devices": "デバイスをロード中...",
        # 转录页
        "realtime_transcript": "リアルタイム文字起こし",
        "stop": "停止",
        "waiting": "文字起こし結果を待っています...",
        # 错误
        "no_device_selected": "デバイスを選択してください",
        "start_failed": "起動に失敗しました",
        "audio_error": "オーディオエラー",
        "audio_capture_failed": "オーディオ録音に失敗：",
        # 语言选择
        "language": "言語",
        "language_zh": "简体中文",
        "language_ja": "日本語",
        "language_en": "English",
        # 设置
        "settings": "設定",
        "whisper_servers": "Whisper サーバーリスト",
        "add_server": "追加",
        "remove_server": "削除",
        "set_default": "デフォルトに設定",
        "new_server_url": "新サーバー URL:",
        "server_name_optional": "名前（任意）:",
        "server_url_placeholder": "http://your-server:port",
        "server_name_placeholder": "任意：サーバー名",
        "vad_parameters": "VAD パラメータ",
        "silence_duration": "無音時間",
        "max_chunk_duration": "最大チャンク時間",
        "min_chunk_duration": "最小チャンク時間",
        "padding_duration": "パディング時間",
        "restore_defaults": "デフォルトに戻す",
        # 本地 Whisper
        "local_whisper_model": "ローカル Whisper モデル (GPU)",
        "local_model_size": "モデルタイプ",
        "local_port": "サービスポート",
        "local_device": "実行デバイス",
        "local_device_auto": "自動検出 (GPU 優先)",
        "local_device_gpu": "GPU (最高性能)",
        "local_device_cpu": "CPU (互換モード)",
        "local_cpu_warning": "注意：Whisper モデルは音声ソースの品質に非常に敏感です。マイクに近づいて話すか、イヤホン付属のマイクを使用した場合にのみ、良好な認識精度が得られます。この問題は、今後 Smoco のリアルタイム音声文字起こし機能を統合することで改善される予定です。",
        "local_start": "サービス開始",
        "local_stop": "サービス停止",
        "local_status": "ステータス",
        "local_status_stopped": "停止中",
        "local_status_starting": "起動中...",
        "local_status_running": "実行中",
        "local_status_downloading": "モデルダウンロード中...",
        "local_already_running": "サービスは既に実行中です",
        "local_starting": "サービスを起動中",
        "local_running": "実行中",
        "local_stopped": "停止済み",
        "local_start_timeout": "起動タイムアウト、ログを確認してください",
        "local_server_name": "ローカル Whisper サービス",
        "local_model_small_label": "Small OpenVINO (INT8)",
        "local_model_small_desc": "高速・低負荷、日常向け",
        "local_model_turbo_label": "Large v3-Turbo (INT8)",
        "local_model_turbo_desc": "高精度、より多くの VRAM/メモリが必要",
        "local_model_recommended": "推奨",
        "local_model_missing": "選択したモデルが見つかりません。再インストールまたはモデルディレクトリを確認してください",
        "local_model_switch_restart": "モデルを切り替えました。サービスを再起動すると反映されます",
        "save": "保存",
        "cancel": "キャンセル",
        "hint_enter_url": "サーバー URL を入力してください",
        "hint_server_exists": "このサーバーは既に存在します",
        "hint_cannot_delete_default": "デフォルトサーバーは削除できません。先に他のサーバーをデフォルトに設定してください",
        "hint_select_server": "サーバーを選択してください",
        "hint_need_server": "少なくとも1つの Whisper サーバーを追加してください",
        # 启动对话框
        "startup_title": "文字起こし開始",
        "select_whisper_server": "Whisper サーバーを選択:",
        "current_server": "現在のサーバー:",
        "service_status": "サーバー状態",
        "service_checking": "確認中...",
        "checking": "確認中",
        "service_normal": "正常",
        "service_error": "エラー",
        "service_timeout": "接続タイムアウト",
        "service_connection_error": "サーバーに接続できません",
        "service_check_failed": "確認失敗",
        "select_language": "文字起こし言語を選択:",
        "select_translate_language": "翻訳言語を選択:",
        "enable_prompt": "プロンプトを有効化（速度低下の可能性あり）",
        "japanese": "日本語",
        "english": "英語",
        "chinese": "中国語",
        "speaker": "スピーカー",
        "microphone": "マイク",
        # LLM 配置
        "llm_config": "LLM 設定",
        "llm_base_url": "Base URL:",
        "llm_api_key": "API Key:",
        "llm_model": "モデル ID:",
        "llm_validate": "設定を確認",
        "llm_validating": "確認中...",
        "llm_config_ok": "設定正確",
        "llm_config_error": "設定エラー",
        "llm_config_incomplete": "設定が不完全",
        "llm_config_timeout": "接続タイムアウト",
        "llm_config_connection_error": "サーバーに接続できません",
        "llm_config_failed": "確認失敗",
        "llm_not_configured": "LLM が未設定",
        "llm_invalid_response": "レスポンス形式が無効",
        "llm_json_parse_error": "JSON 解析失敗",
        "llm_timeout": "翻訳タイムアウト",
        "llm_base_url_placeholder": "https://api.openai.com/v1",
        "llm_api_key_placeholder": "sk-...",
        "llm_model_placeholder": "gpt-4o",
        "llm_instructions": "OpenAI 互換 API サービス",
        "llm_use_default": "デフォルトを使用",
        "smoco_use_default": "デフォルトを使用",
        "smoco_source_name": "Smoco クラウド ASR",
        "smoco_stt_config": "Smoco クラウド STT",
        "smoco_instructions": "説明：smoco アカウントでクラウドストリーミング音声認識（socket.io）にログイン",
        "smoco_host": "サーバーアドレス",
        "smoco_host_placeholder": "https://dx-smoco-dev.sony.com.cn",
        "smoco_email": "アカウント (Email)",
        "smoco_password": "パスワード",
        "smoco_use_punctuator": "自動句読点",
        "smoco_insecure_ssl": "SSL 証明書検証を無効化（内部 CA）",
        "smoco_validate": "ログイン確認",
        "smoco_validating": "確認中...",
        "smoco_validate_ok": "ログイン成功",
        "smoco_validate_failed": "ログイン失敗",
        "smoco_not_configured": "Smoco アカウントが未設定です。設定で入力してください",
        "cannot_close_while_transcribing": "文字起こし中です。先に文字起こしを停止してから閉じてください",
        "smoco_account_unavailable": "Smoco アカウントは現在利用できません。設定のアカウントまたはネットワークを確認してください",
        "llm_context_length": "翻訳コンテキスト数:",
        "llm_context_length_tooltip": "翻訳時に前のN条の転録テキストをコンテキストとして使用、デフォルト5条",
        "start": "開始",
        "service_unavailable": "サービス利用不可",
        "service_unavailable_msg": "API サービスに接続できません。他のサーバーを選択するか設定を確認してください",
        # 转录交互
        "copy_transcript": "テキストをコピー",
        "copy_with_timestamp": "タイムスタンプ付きでコピー",
        # 通用
        "hint": "ヒント",
        "warning": "警告",
        "error": "エラー",
        "success": "成功",
        "failed": "失敗",
        "loading": "ロード中...",
        "please_wait": "お待ちください",
        # 错误消息
        "need_server_config": "設定で Whisper サーバーを追加してください",
        "unable_to_start": "開始できません：",
        # 転録履歴
        "history": "履歴",
        "history_title": "転録履歴",
        "history_back": "戻る",
        "history_loading": "ロード中...",
        "history_no_sessions": "転録履歴がありません",
        "history_no_entries": "このセッションには転録がありません",
        "history_entry_count": "{n} 件",
        "history_prev_page": "前のページ",
        "history_next_page": "次のページ",
        "history_page_n_of_m": "{n} / {m} ページ",
        "history_export": "エクスポート",
        "history_export_this": "このセッションをエクスポート",
        "history_export_format": "エクスポート形式:",
        "history_export_done": "エクスポート完了: {path}",
        "history_copy_original": "原文をコピー",
        "history_copy_translation": "訳文をコピー",
        "history_copy_done": "クリップボードにコピーしました",
        "history_no_translation": "（翻訳なし）",
        "history_close": "閉じる",
        "history_time_column": "時間",
        "history_original_column": "原文",
        "history_translation_column": "訳文",
        "history_detail_title": "転録詳細 #{id}",
        # ログビューア
        "logs": "ログ",
        "logs_title": "アプリログ",
        "refresh_logs": "更新",
        "open_log_dir": "ログフォルダを開く",
        "error_logs_only": "エラーログのみ",
        "no_logs": "ログファイルがありません",
        "loading_logs": "ロード中...",
        "log_status": "最新 {shown} / {total} 行表示 · ファイルサイズ {size}",
    },
    "en": {
        # Main window
        "window_title": "Smoco Desktop",
        # Device selection page
        "select_device": "Select Audio Device",
        "select_device_desc": "Select the speaker device to capture:",
        "start_transcription": "Start Transcription",
        "refresh_devices": "Refresh Device List",
        "no_devices": "No audio devices found",
        "loading_devices": "Loading devices...",
        # Transcript page
        "realtime_transcript": "Real-time Transcript",
        "stop": "Stop",
        "waiting": "Waiting for transcription...",
        # Errors
        "no_device_selected": "Please select a device first",
        "start_failed": "Start Failed",
        "audio_error": "Audio Error",
        "audio_capture_failed": "Audio capture failed:",
        # Language selection
        "language": "Language",
        "language_zh": "简体中文",
        "language_ja": "日本語",
        "language_en": "English",
        # Settings
        "settings": "Settings",
        "whisper_servers": "Whisper Server List",
        "add_server": "Add",
        "remove_server": "Remove",
        "set_default": "Set as Default",
        "new_server_url": "New Server URL:",
        "server_name_optional": "Name (Optional):",
        "server_url_placeholder": "http://your-server:port",
        "server_name_placeholder": "Optional: Server name",
        "vad_parameters": "VAD Parameters",
        "silence_duration": "Silence Duration",
        "max_chunk_duration": "Max Chunk Duration",
        "min_chunk_duration": "Min Chunk Duration",
        "padding_duration": "Padding Duration",
        "restore_defaults": "Restore Defaults",
        # Local Whisper
        "local_whisper_model": "Local Whisper Model (GPU)",
        "local_model_size": "Model Type",
        "local_port": "Service Port",
        "local_device": "Runtime Device",
        "local_device_auto": "Auto Detect (GPU Preferred)",
        "local_device_gpu": "GPU (Best Performance)",
        "local_device_cpu": "CPU (Compatibility Mode)",
        "local_cpu_warning": "Note: Whisper is very sensitive to audio source quality — good recognition is only achieved when speaking close to the microphone, or when using a headset microphone. This will improve once Smoco's real-time speech transcription is integrated.",
        "local_start": "Start Service",
        "local_stop": "Stop Service",
        "local_status": "Status",
        "local_status_stopped": "Stopped",
        "local_status_starting": "Starting...",
        "local_status_running": "Running",
        "local_status_downloading": "Downloading model...",
        "local_already_running": "Service already running",
        "local_starting": "Starting service",
        "local_running": "Running",
        "local_stopped": "Stopped",
        "local_start_timeout": "Start timeout, please check logs",
        "local_server_name": "Local Whisper Service",
        "local_model_small_label": "Small OpenVINO (INT8)",
        "local_model_small_desc": "Fast and lightweight, for daily use",
        "local_model_turbo_label": "Large v3-Turbo (INT8)",
        "local_model_turbo_desc": "Higher accuracy, needs more VRAM/RAM",
        "local_model_recommended": "Recommended",
        "local_model_missing": "Selected model files not found. Please reinstall or check the model directory",
        "local_model_switch_restart": "Model switched — restart the service to apply",
        "save": "Save",
        "cancel": "Cancel",
        "hint_enter_url": "Please enter server URL",
        "hint_server_exists": "This server already exists",
        "hint_cannot_delete_default": "Cannot delete default server, please set another server as default first",
        "hint_select_server": "Please select a server first",
        "hint_need_server": "Please add at least one Whisper server",
        # Startup dialog
        "startup_title": "Start Transcription",
        "select_whisper_server": "Select Whisper Server:",
        "current_server": "Current Server:",
        "service_status": "Service Status",
        "service_checking": "Checking...",
        "checking": "Checking",
        "service_normal": "Normal",
        "service_error": "Error",
        "service_timeout": "Connection Timeout",
        "service_connection_error": "Cannot connect to server",
        "service_check_failed": "Check Failed",
        "select_language": "Select Transcription Language:",
        "select_translate_language": "Select Translation Language:",
        "enable_prompt": "Enable prompt (may slow down recognition)",
        "japanese": "Japanese",
        "english": "English",
        "chinese": "Chinese",
        "speaker": "Speaker",
        "microphone": "Microphone",
        # LLM Configuration
        "llm_config": "LLM Configuration",
        "llm_base_url": "Base URL:",
        "llm_api_key": "API Key:",
        "llm_model": "Model ID:",
        "llm_validate": "Validate",
        "llm_validating": "Validating...",
        "llm_config_ok": "Configuration OK",
        "llm_config_error": "Configuration Error",
        "llm_config_incomplete": "Configuration Incomplete",
        "llm_config_timeout": "Connection Timeout",
        "llm_config_connection_error": "Cannot connect to server",
        "llm_config_failed": "Validation Failed",
        "llm_not_configured": "LLM Not Configured",
        "llm_invalid_response": "Invalid Response Format",
        "llm_json_parse_error": "JSON Parse Error",
        "llm_timeout": "Translation Timeout",
        "llm_base_url_placeholder": "https://api.openai.com/v1",
        "llm_api_key_placeholder": "sk-...",
        "llm_model_placeholder": "gpt-4o",
        "llm_instructions": "OpenAI Compatible API Service",
        "llm_use_default": "Use Default",
        "smoco_use_default": "Use Default",
        "smoco_source_name": "Smoco Cloud ASR",
        "smoco_stt_config": "Smoco Cloud STT",
        "smoco_instructions": "Note: log in to cloud streaming ASR (socket.io) with your smoco account",
        "smoco_host": "Server Host",
        "smoco_host_placeholder": "https://dx-smoco-dev.sony.com.cn",
        "smoco_email": "Account (Email)",
        "smoco_password": "Password",
        "smoco_use_punctuator": "Auto punctuation",
        "smoco_insecure_ssl": "Disable SSL verification (internal CA)",
        "smoco_validate": "Verify Login",
        "smoco_validating": "Verifying...",
        "smoco_validate_ok": "Login OK",
        "smoco_validate_failed": "Login failed",
        "smoco_not_configured": "Smoco account not configured. Please fill in settings.",
        "cannot_close_while_transcribing": "Transcription is in progress. Please stop it before closing the window",
        "smoco_account_unavailable": "Smoco account is currently unavailable. Please check the account or network in settings",
        "llm_context_length": "Context Entries:",
        "llm_context_length_tooltip": "Number of previous transcript entries to use as context for translation, default 5",
        "start": "Start",
        "start": "Start",
        "service_unavailable": "Service Unavailable",
        "service_unavailable_msg": "Cannot connect to API service, please select another server or check configuration",
        # Transcript interaction
        "copy_transcript": "Copy Transcript Text",
        "copy_with_timestamp": "Copy with Timestamp",
        # General
        "hint": "Hint",
        "warning": "Warning",
        "error": "Error",
        "success": "Success",
        "failed": "Failed",
        "loading": "Loading...",
        "please_wait": "Please wait",
        # Error messages
        "need_server_config": "Please add a Whisper server in settings first",
        "unable_to_start": "Unable to start:",
        # Transcript history
        "history": "History",
        "history_title": "Transcript History",
        "history_back": "Back",
        "history_loading": "Loading...",
        "history_no_sessions": "No transcript history yet",
        "history_no_entries": "No entries in this session",
        "history_entry_count": "{n} entries",
        "history_prev_page": "Previous",
        "history_next_page": "Next",
        "history_page_n_of_m": "Page {n} / {m}",
        "history_export": "Export",
        "history_export_this": "Export this session",
        "history_export_format": "Export format:",
        "history_export_done": "Exported to: {path}",
        "history_copy_original": "Copy Original",
        "history_copy_translation": "Copy Translation",
        "history_copy_done": "Copied to clipboard",
        "history_no_translation": "(no translation)",
        "history_close": "Close",
        "history_time_column": "Time",
        "history_original_column": "Original",
        "history_translation_column": "Translation",
        "history_detail_title": "Entry #{id}",
        # Logs
        "logs": "Logs",
        "logs_title": "Application Logs",
        "refresh_logs": "Refresh",
        "open_log_dir": "Open Log Folder",
        "error_logs_only": "Error logs only",
        "no_logs": "No log files",
        "loading_logs": "Loading...",
        "log_status": "Showing last {shown} / {total} lines · file size {size}",
    },
}

LANGUAGES = [
    ("zh", "简体中文"),
    ("ja", "日本語"),
    ("en", "English"),
]


class I18n:
    """国际化管理器"""

    def __init__(self):
        self._current_lang = "zh"

    @property
    def current_lang(self):
        return self._current_lang

    def set_language(self, lang: str):
        """设置当前语言"""
        if lang in TRANSLATIONS:
            self._current_lang = lang

    def t(self, key: str) -> str:
        """翻译文本"""
        return TRANSLATIONS.get(self._current_lang, {}).get(key, key)

    def __getitem__(self, key: str) -> str:
        """支持 i18n[key] 语法"""
        return self.t(key)


# 全局实例
i18n = I18n()
